...
class Users::SessionsControllerTest < ActionDispatch::IntegrationTest
  ...
  test 'create action does not create event if login fails' do
  ...
  test 'destroy action does not create event if user not signed in' do
  ...
  test 'overrides create and destroy methods' do
    ...
    expected_overrides = %i[create destroy]

    expected_overrides.each do |method|
      assert_includes overridden_methods, method, "Users::SessionsController should override #{method}"
    end
  end
  ...
  test 'handles event creation failure gracefully' do
    ...
    Event.define_singleton_method(:create) { raise StandardError, 'Event creation failed' }
  ...
  test 'login form includes CSRF protection' do
    assert ActionController::Base.forgery_protection_strategy.present?
    assert Rails.application.config.action_controller.allow_forgery_protection != false || Rails.env.test?
  end
