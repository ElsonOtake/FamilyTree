...
unless Couple.any?
  ...
  csv.each do |row|
    ...
    puts row['id']
    couple.id = row['id']
    couple.person1_id = row['person1_id']
    couple.person2_id = row['person2_id']
  ...
  csv = CSV.parse(csv_text, headers: true, encoding: 'ISO-8859-1', col_sep: ';')
  csv.each do |row|
    couple = Couple.find(row['couple_id'])
    couple.people << Person.find(row['person_id'])
    couple.save
  end
end
...
unless User.any?
  ...
  csv.each do |row|
    ...
    user.password = row['id'] == 1 ? 'foobar' : '123456'
    user.name = row['name']
