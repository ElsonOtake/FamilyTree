class FamilyMailer < ApplicationMailer
  ...
  def couple_deleted
    ...
    mail to: "no-reply@example.com", subject: "Couple deleted: #{@person_1_name} & #{@person_2_name} (#{@id}) by #{@user.name}"
  end
  ...
  def child_deleted
    ...
    mail to: "no-reply@example.com", subject: "Child deleted: #{@child.name} (#{@child.id}) by #{@user.name}"
  end
end
